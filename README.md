...
- `aicrawl sync web --provider chatgpt|claude --cdp-url <url>` attaches to an already authenticated browser target and fetches bounded recent conversation list/detail payloads from page context under `chatgpt_web` or `claude_web`. Provider update cursors are stored when available so later syncs can skip older list candidates and report no-change checks without rewriting the archive.
