...
func (a *App) reconcile(ctx context.Context, globals globalOptions, args []string) error {
	...
	if err := writeTextLine(a.stdout, "%s reconciliation: %d/%d conversations archived, %d/%d messages archived",
		report.Provider,
...
func reconcileSource(ctx context.Context, db *sql.DB, path, provider string) (reconcileReport, error) {
	...
	header, err := streamImportSource(path, provider, func(conversation archive.Conversation, warnings []string) error {
		...
		for _, message := range conversation.Messages {
			exists, err := rowExists(ctx, db, `select 1 from messages where id = ? limit 1`, message.ID)
			if err != nil {
				return err
			}
			if exists {
	...
	if report.MissingConversations > 0 || report.MissingMessages > 0 {
		report.NextStep = "Run `aicrawl import` with this official export to backfill missing archive rows."
	}
	return report, nil
}
