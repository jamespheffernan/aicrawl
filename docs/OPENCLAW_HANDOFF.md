...
- browser-profile sync preflight, ChatGPT macOS app cache ID discovery, and launch orchestration under `internal/sync/`.
- FTS query handling in `internal/textnorm/`.
- public docs and fixture hygiene.

## Privacy Posture

v0.1 imports local official exports, captured ChatGPT/Claude web detail payload files, bounded live CDP page-context ChatGPT/Claude payloads, ChatGPT macOS app cache conversation IDs as live-web detail seeds, OpenClaw/Codex/Gemini/Claude Code local session files or roots, Cursor `store.db`/`state.vscdb` chat stores or roots, and Hermes `state.db`/session JSON/JSONL stores or roots. It can also run read-only web-sync preflight, launch a dedicated provider browser profile for web sync, write macOS LaunchAgent plists for recurring web sync, and run the aggregate-only live provider smoke documented in `docs/LIVE_PROVIDER_SMOKE.md`. It does not make network calls for file import, sync source import, ChatGPT app cache ID discovery, profile-only sync preflight, search, SQL, Markdown export, LaunchAgent generation, CrawlBar manifest generation, or the local-provider import paths. `sync web --dry-run --cdp-url` and `scripts/live-provider-smoke.sh --mode dry-run` make list-only same-origin provider calls from an already open attached browser page to count candidates unless `--chatgpt-app-cache` is provided, in which case dry-run only counts filename-derived local IDs. Write-capable ChatGPT app-cache-seeded sync fetches those exact details through the same authenticated ChatGPT web page context. It does not implement session-token scraping, browser automation to click export buttons, unattended provider login, or cloud storage.
