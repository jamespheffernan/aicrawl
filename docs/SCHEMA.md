...
Provider registry keyed by provider ID. v0.1 uses `claude`, `chatgpt`, `openclaw`, `codex`, `gemini`, and `claude-code`.
...
- `id`: stable import ID.
- `source_kind`: provider source type such as `claude_export`, `chatgpt_web`, `openclaw_jsonl`, `codex_jsonl`, `gemini_cli`, or `claude_code_jsonl`.
- `provider`: provider ID.
...
Local summary table for source-kind status. v0.1 writes it during imports, including `sync web --source` and live `sync web --cdp-url` imports with source kinds such as `chatgpt_web` and `claude_web`, and reads it during `sync web --dry-run` to report freshness.
