...
Provider registry keyed by provider ID. v0.1 uses `claude` and `chatgpt`.
...
- `id`: stable import ID.
- `source_kind`: provider source type such as `claude_export` or `chatgpt_export`.
- `provider`: provider ID.
...
Local summary table for source-kind status. v0.1 uses it for import/status bookkeeping, not network sync.
