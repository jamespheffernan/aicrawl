...
aicrawl schedule launchd --local-defaults --interval-minutes 5 --json
```
...
With `--local-defaults`, the command checks standard local roots and writes import LaunchAgents only for roots that currently exist:

- `~/.openclaw/agents/main/sessions` as `openclaw`
- `~/.codex/sessions` as `codex`
- `~/.gemini` as `gemini`
- `~/.claude/projects` as `claude-code`
- `~/Library/Application Support/Claude` as `claude-code` with a distinct Claude desktop label
- `~/Library/Application Support/Cursor` as `cursor`
- `~/.hermes` as `hermes`

JSON output includes generated `results` and skipped missing or unavailable roots. `--local-defaults` cannot be combined with per-provider, `--import-path`, web-sync, `--out`, or `--label` options.
